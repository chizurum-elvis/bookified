...
export async function parsePDFFile(file: File) {
  try {
    ...
    // 1. Get the HTML element reference
    const canvasElement = document.getElementById('my-canvas') as HTMLCanvasElement;
    // Or if using a React useRef: const canvasElement = canvasRef.current;

    if (canvasElement) {
      // 2. Get the 2D context from that element
      const context = canvasElement.getContext('2d');

    if (context) {
      const renderContext = {
        canvasContext: context,       // Pass the 2D context here
        canvas: canvasElement,        // Pass the HTML element here (Fixes your current error)
        viewport: viewport,
      };
